 ...
 *
 */
var idUrl = 'module-usersui';
var idForm = 'module-usersui-form';
var className = 'ModuleUsersUI';
...
var ModuleUsersUI = {
  ...
  validateRules: {
    textField: {
      ...
      rules: [{
        type: 'empty',
        prompt: globalTranslate.module_usersuiValidateValueIsEmpty
      }]
    },
    areaField: {
  ...
  changeStatus: function changeStatus(status) {
    switch (status) {
      case 'Connected':
        ...
        window[className].$moduleStatus.html(globalTranslate.module_usersuiConnected);
        break;

      case 'Disconnected':
        window[className].$moduleStatus.removeClass('green').removeClass('red').addClass('grey');
        window[className].$moduleStatus.html(globalTranslate.module_usersuiDisconnected);
        break;

      case 'Updating':
        window[className].$moduleStatus.removeClass('green').removeClass('red').addClass('grey');
        window[className].$moduleStatus.html("<i class=\"spinner loading icon\"></i>".concat(globalTranslate.module_usersuiUpdateStatus));
        break;
